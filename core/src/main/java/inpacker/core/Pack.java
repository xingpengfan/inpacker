...
public class Pack {
    ...
    void processing() {
        if (status != INITIATED)
            throw new IllegalStateException("pack status is not INITIATED");
        status = PROCESSING;
    }

    void failed() {
        if (status != PROCESSING)
            throw new IllegalStateException("only PROCESSING pack could be failed");
        status = FAILED;
    }
    ...
    void done(File packFile) {
        ...
        if (!packFile.exists())
            throw new IllegalArgumentException("packFile does not exist");
        if (status != PROCESSING)
            throw new IllegalStateException("only PROCESSING pack could be done");
        file = packFile;
    ...
    void newSuccessItem(PackItem item) {
        if (status != PROCESSING)
            throw new IllegalStateException("items could be added only in PROCESSING pack");
        addedItems.add(item);
    }
